...
class ProjectController extends Controller {
	...
	public function single( $project_path ) {
		$project = Project::by_path( $project_path );

		if ( ! $project ) {
			return abort(404);
		}

		Head::setTitle( sprintf( _('%s project '), $project->name ) );

		$sub_projects = $project->sub_projects();
		$translation_sets = TranslationSet::by_project_id( $project->id );
		...
		$translation_sets->sort( function( $a, $b ) {
			return( $a->current_count < $b->current_count );
		});
		//$translation_sets = apply_filters( 'translation_sets_sort', $translation_sets );

		return view( 'project', [ 'project' => $project, 'sub_projects' => $sub_projects, 'translation_sets' => $translation_sets ] );
	}
