...
use Gwnobots\LaravelHead\LaravelHeadFacade as Head;
use App\Models\Project;
use App\Models\TranslationSet;
use GP_Locales;
use stdClass;
...
class LocaleController extends Controller {
	...
	public function index() {
		Head::setTitle( _('Locales') );

		if ( isset( $_GET['all'] ) ) {
			$locales = GP_Locales::locales();
		}
		else {
			$existing_locales = TranslationSet::existing_locales();
			$locales = array();
		...
		return view( 'locales', [ 'locales' => $locales ] );
	}
	...
	public function single( $locale_slug, $current_set_slug = 'default' ) {
		...
		Head::setTitle( sprintf( _( 'Projects translated to %s &lt; GlotPress' ),  $locale->english_name ) );

		$sets = TranslationSet::by_locale( $locale_slug );
		...
		foreach ( $sets as $set ) {
			...
			if ( isset( $projects[ $set->project_id ] ) ) {
				...
				$set_project = Project::find( $set->project_id );
			...
			foreach ( $set_slugs as $set ) {
				if ( 'default' == $set->slug ) {
					if ( 'default' != $current_set_slug ) {
						$set_list[ $set->slug ] = gp_link_get( gp_url_join( '/languages', $locale->slug ), _( 'Default' ) );
					} else {
						$set_list[ $set->slug ] = _( 'Default' );
					}
				} else {
		...
		return view( 'locale', get_defined_vars() );
	}
